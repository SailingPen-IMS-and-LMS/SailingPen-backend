...
@UseGuards(JwtAuthGuard, RolesGuard)
  ...
  createQuestion(@Req() req:Request,@Param('quizId') quizId: string, @Body() questionData: CreateQuestionDto) {
    const user= req.user as AuthenticatedUser;
    const userId=user.sub;
    return this.quizzesService.createQuestion(userId,quizId, questionData);
  }
