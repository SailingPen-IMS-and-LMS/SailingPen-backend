...
import { Body, Controller, Post, Get, UseGuards, Req, Query, ForbiddenException } from '@nestjs/common';
...
export class TutionClassesController {
    ...
    async enrollStudentToClassByStudent(@Body() enrollToClassDto: EnrollToClassDto, @Req() req: Request) {
        ...
        if (!foundStudent || foundStudent.student_id !== student_id) {
            throw new ForbiddenException("You are not allowed to enroll other students")
        }
        return this.tutionClassesService.enrollStudent(enrollToClassDto)
    }
    ...
    @UseGuards(JwtAuthGuard)
    @Get("not-enrolled")
    getNotEnrolledClassesOfStudentByStudent(@Req() req: Request, @Query('tutor_id') tutorId: string) {
        const user = req.user as AuthenticatedUser
        const userId = user.sub
        return this.tutionClassesService.getClassesForTheStudentToEnrollIn(userId, tutorId)
    } 
}
